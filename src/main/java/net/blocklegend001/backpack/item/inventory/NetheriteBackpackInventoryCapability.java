...
public class NetheriteBackpackInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == SimplebackpackModItems.NETHERITE_BACKPACK.get()) {
			if (Minecraft.getInstance().screen instanceof NetheriteBackpackGUIScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(90) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != SimplebackpackModItems.NETHERITE_BACKPACK.get();
			}
